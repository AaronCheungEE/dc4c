...
static int Pm_Hzbat_Batches_Tasks_EraseTailSpace(Pm_Hzbat_Batches_Tasks *_erase_data)
{
	...
	ERASE_TAIL_SPACE(_erase_data->ip);
	return(0);
}
...
int DB_pm_hzbat_batches_tasks_open_select_by_schedule_name( char *schedule_name__0,Select_Info *_a_sInfo)
{
		...
		R_pm_hzbat_batches_tasks.ip,41,DT_STR,
		&R_pm_hzbat_batches_tasks.port,4,DT_ITG,
...
int DB_pm_hzbat_batches_tasks_update_for_begin_datetime_and_end_datetime_and_ip_and_port_and_progress_and_error_and_status_by_schedule_name_and_batch_name_and_order_index( char *schedule_name__8,char *batch_name__9,int order_index__10,Pm_Hzbat_Batches_Tasks *_a_data)
{
		...
		"3",R_pm_hzbat_batches_tasks.ip,41,DT_STR,
		"4",&R_pm_hzbat_batches_tasks.port,4,DT_ITG,
		"5",&R_pm_hzbat_batches_tasks.progress,4,DT_ITG,
		"6",&R_pm_hzbat_batches_tasks.error,4,DT_ITG,
		"7",&R_pm_hzbat_batches_tasks.status,4,DT_ITG,
		"8",schedule_name__8,65,DT_STR,
		"9",batch_name__9,65,DT_STR,
		"10",&order_index__10,4,DT_ITG,
		NULL);
        return(r);
}
...
int DB_pm_hzbat_batches_tasks_update_for_end_datetime_and_progress_and_error_and_status_by_schedule_name_and_batch_name_and_order_index( char *schedule_name__5,char *batch_name__6,int order_index__7,Pm_Hzbat_Batches_Tasks *_a_data)
{
		...
		WHERE schedule_name = ? AND \
			batch_name = ? AND \
			order_index = ?",
		"1",R_pm_hzbat_batches_tasks.end_datetime,20,DT_STR,
		"2",&R_pm_hzbat_batches_tasks.progress,4,DT_ITG,
		"3",&R_pm_hzbat_batches_tasks.error,4,DT_ITG,
		"4",&R_pm_hzbat_batches_tasks.status,4,DT_ITG,
		"5",schedule_name__5,65,DT_STR,
		"6",batch_name__6,65,DT_STR,
		"7",&order_index__7,4,DT_ITG,
		NULL);
        return(r);
}
...
int DB_pm_hzbat_batches_tasks_add( Pm_Hzbat_Batches_Tasks *_a_data)
{
		...
		"8",R_pm_hzbat_batches_tasks.ip,41,DT_STR,
		"9",&R_pm_hzbat_batches_tasks.port,4,DT_ITG,
		"10",&R_pm_hzbat_batches_tasks.progress,4,DT_ITG,
		"11",&R_pm_hzbat_batches_tasks.error,4,DT_ITG,
		"12",&R_pm_hzbat_batches_tasks.status,4,DT_ITG,
		NULL);
        return(r);
}
...
int DB_pm_hzbat_batches_tasks_debug_log(char *reason,Pm_Hzbat_Batches_Tasks *adata, char *filename, int line_no,int priority)
{
	...
	hzb_log_print(filename,line_no,priority, "ip: %s", adata->ip );
	hzb_log_print(filename,line_no,priority, "port: %d", adata->port );
