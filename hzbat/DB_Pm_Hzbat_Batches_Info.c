...
static int Pm_Hzbat_Batches_Info_EraseTailSpace(Pm_Hzbat_Batches_Info *_erase_data)
{
	...
	ERASE_TAIL_SPACE(_erase_data->pretask_ip);
	return(0);
}
...
int DB_pm_hzbat_batches_info_open_select_by_schedule_name( char *schedule_name__0,Select_Info *_a_sInfo)
{
		...
		R_pm_hzbat_batches_info.pretask_ip,41,DT_STR,
		&R_pm_hzbat_batches_info.pretask_port,4,DT_ITG,
...
int DB_pm_hzbat_batches_info_update_for_end_datetime_and_progress_by_schedule_name_and_batch_name( char *schedule_name__3,char *batch_name__4,Pm_Hzbat_Batches_Info *_a_data)
{
		...
		WHERE schedule_name = ? AND \
			batch_name = ?",
		"1",R_pm_hzbat_batches_info.end_datetime,20,DT_STR,
		"2",&R_pm_hzbat_batches_info.progress,4,DT_ITG,
		"3",schedule_name__3,65,DT_STR,
		"4",batch_name__4,65,DT_STR,
		NULL);
        return(r);
}
...
int DB_pm_hzbat_batches_info_update_for_pretask_ip_and_pretask_port_and_pretask_progress_and_pretask_error_and_pretask_status_by_schedule_name_and_batch_name( char *schedule_name__6,char *batch_name__7,Pm_Hzbat_Batches_Info *_a_data)
{
		...
		WHERE schedule_name = ? AND \
			batch_name = ?",
		"1",R_pm_hzbat_batches_info.pretask_ip,41,DT_STR,
		"2",&R_pm_hzbat_batches_info.pretask_port,4,DT_ITG,
		"3",&R_pm_hzbat_batches_info.pretask_progress,4,DT_ITG,
		"4",&R_pm_hzbat_batches_info.pretask_error,4,DT_ITG,
		"5",&R_pm_hzbat_batches_info.pretask_status,4,DT_ITG,
...
int DB_pm_hzbat_batches_info_debug_log(char *reason,Pm_Hzbat_Batches_Info *adata, char *filename, int line_no,int priority)
{
	...
	hzb_log_print(filename,line_no,priority, "pretask_ip: %s", adata->pretask_ip );
	hzb_log_print(filename,line_no,priority, "pretask_port: %d", adata->pretask_port );
